 ...
 * Tasks are executed through the {@link BlightedSMP} plugin scheduler and can be
...
public final class LifecycleTaskManager {
    ...
    private static final class ScheduledTask {
        ...
        private void schedule(LifecycleTaskManager manager) {
            ...
            if (repeating) {
                current = factory.get();
                current.runTaskTimer(BlightedSMP.getInstance(), delayTicks, periodTicks);
            } else {
                ...
                current.runTaskLater(BlightedSMP.getInstance(), delayTicks);
            }
        }

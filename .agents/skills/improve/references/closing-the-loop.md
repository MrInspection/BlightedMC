...
- The plan file exists and its dependencies show DONE in `<plan-dir>/README.md` (where `<plan-dir>` is the selected root directory, e.g. `plans/` or `advisor-plans/`). If not: stop, name the missing dependency.
...
1. **The full plan file text, inlined.** The worktree contains only committed files — if `<plan-dir>/` is uncommitted, the executor can't read it. Never assume; always inline.
...
> One override: SKIP the plan's instruction to update `<plan-dir>/README.md` —
...
## `reconcile` — keep `<plan-dir>/` alive

Process what happened since the last session. Read `<plan-dir>/README.md` and every plan file, then per status:

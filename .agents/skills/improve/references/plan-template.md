...
File naming: `<plan-dir>/NNN-short-slug.md` (where `<plan-dir>` is the selected root directory, e.g., `plans/` or `advisor-plans/`), numbered in recommended execution order.
...
- [ ] `<plan-dir>/README.md` status row updated

...
name: clean-commit
description: >-
  Drafts commit messages in `category(scope): message` conventional-commit
  ...
  restated diffs, emoji, exclamation points). Use ONLY when the user
  explicitly asks to write, generate, or clean up a commit message, or
  explicitly asks to commit. Never trigger this proactively after finishing
  an unrelated coding task, and never run `git commit` (or stage/push for
  that purpose) without the user's separate, explicit approval of the exact
  message shown — drafting and executing are two different steps requiring
  two different asks. NOT for PR descriptions or changelogs — those need
  more context than a commit subject line and are a different deliverable.
---

# Clean Commit

A commit message's job is to tell the next person (often future-you) *why*
a change happened, in one line, using the fewest words that don't lose
...
and it never runs `git commit` on its own say-so.

## Approval gate — read this first

- **Never invoke this skill unprompted.** Finishing a coding task is not, by
  itself, a reason to draft or suggest a commit message. Wait for the user
  to ask for one, explicitly, in this turn.
- **Drafting is not committing.** A request like "commit this" or "write a
  commit message for these changes" means: inspect the diff, draft the
  message, show it. It does not mean run `git commit`.
- **Executing requires a second, explicit approval of the shown message** —
  something like "yes, commit it," "use that," or "run it." A vague
  continuation of the conversation is not approval. If there's any doubt
  whether the user approved the *specific message shown* versus just the
  idea of committing in general, ask before running the command.
- This gate applies regardless of how small or routine the change looks.
  There is no change trivial enough to skip it.
...
- Scope should match a real module/directory/component name already used in
  ...
  that doesn't match what the codebase calls that area. If the repo has its
  own naming rules (e.g. a banned-abbreviations list in its agent
  guidelines), the scope name follows those too — a commit scope is still
  an identifier a reader has to parse.
...
**Then stop.** Do not run `git commit`, `git add`, or `git push` after
showing the message. Wait for the user's explicit, separate approval of
that specific message before executing anything.
